...
public class LambdaHomework3 {
  ...
  static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
    ...
    int[][] distances = new int[LIST_SIZE][LIST_SIZE];
     ...
     StreamSupport
             .stream(wordList.stream().spliterator(), parallel)
             .map(x -> wordList // Refactor
                    .stream()
                    .map(y -> Levenshtein.lev(x, y))
                    .mapToInt(Integer::valueOf)
                    .toArray())
            .collect(Collectors.toSet()).toArray(distances);
